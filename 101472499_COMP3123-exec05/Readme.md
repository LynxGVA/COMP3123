...
- Why is `express.Router()` used in Express.js applications, and how does it benefit the code structure?
 It lets us put related routes in separate files. It makes the code cleaner and easier to manage.
...
I would use an error-handling middleware with err, req, res, next. If something goes wrong, the middleware sends a message instead of crashing the server.
...
It means the server uses a port from the environment if available, otherwise it uses 8081. This is useful if the port is assigned by the hosting platform.
